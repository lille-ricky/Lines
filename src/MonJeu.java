...
public class MonJeu implements IJeuDesBilles {
    ...
    /**
     * Verifie si otutes les cases de la grille sont remplies, renvoie true si
     * oui et false sinon.
     *
     * @return true si toutes les cases de la grilles sont remplies, false
     * sinon.
     */
     ...
     * @return le Point representant la position de la bille dans la grille, ou
     * null sinon.
     */
     ...
     * @return la liste des positions ou les nouvelles billes ont ete ajoutees,
     * null sinon
     */
    public java.util.List<java.awt.Point> deplace(int ligD, int colD, int ligA, int colA) {
        if (grilleBilles[ligD][colD] == VIDE) {
            return null;
        }
        ...
        genererNouvellesCouleurs();
        getCordonneesToutesLesCases();
    }

    private java.util.List<java.awt.Point> getCordonneesToutesLesCases() {
        java.util.List<java.awt.Point> toutesLesCases = new java.util.ArrayList<>();

        for (int i = 0; i < NB_LIGNES; i++) {
            for (int j = 0; j < NB_COLONNES; j++) {
                toutesLesCases.add(new java.awt.Point(j, i));
            }
        }
